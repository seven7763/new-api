...
func LogStartupSuccess(startTime time.Time, port string) {
	...
	if SessionCookieSecure == false {
		// log warning if the refresh cookie is not secure
		fmt.Fprintf(gin.DefaultWriter, "\n")
		fmt.Fprintf(gin.DefaultWriter, "  \033[33mWarning: Refresh cookie is not secure. Please set SESSION_COOKIE_SECURE=true in production.\033[0m\n")
		fmt.Fprintf(gin.DefaultWriter, "\n")
	}
