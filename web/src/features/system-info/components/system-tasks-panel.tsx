...
const TYPE_LABEL: Record<string, string> = {
  ...
  invite_rebate_backfill: 'Invite rebate backfill',
}
...
function SystemTasksBody(props: SystemTasksBodyProps) {
    ...
    return (
      <div className='space-y-2 p-4 sm:p-5'>
        {Array.from({ length: 4 }, (_, index) => (
          <Skeleton
            key={`system-task-skeleton-${index}`}
...
export function SystemTasksPanel() {
  ...
  const { activeTasks, historyTasks } = useMemo(() => {
    const active: SystemTask[] = []
    const history: SystemTask[] = []
    for (const task of tasks ?? []) {
      if (isActiveStatus(task.status)) active.push(task)
